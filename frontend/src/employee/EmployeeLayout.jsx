...
const navItems = [
  ...
  { path: '/employee/projects',         label: 'Source Codes',     icon: 'folder',            color: 'from-violet-500 to-purple-600'  },
...
/* ── Color map (Tailwind → CSS gradient) ──────────────────────────── */
const GRADIENTS = {
  ...
  'from-indigo-500 to-violet-600':  'linear-gradient(135deg,#6366f1,#9333ea)',
  ...
  'from-cyan-500 to-sky-500':       'linear-gradient(135deg,#06b6d4,#0ea5e9)',
  'from-lime-500 to-green-600':     'linear-gradient(135deg,#84cc16,#16a34a)',
  'from-slate-500 to-slate-600':    'linear-gradient(135deg,#64748b,#475569)',
...
const getFilteredNavItems = (userRole, permissions, hasAssignedCourses = false) => {
  ...
  return navItems.filter((item) => {
    if (['/employee', '/employee/tasks', '/employee/profile'].includes(item.path)) return true
...
export default function EmployeeLayout() {
  ...
  /* Prevent outer-page scroll when inner scrollable hits edge */
  const handleWheel = (e) => {
    const scrollable = e.target.closest('[style*="overflow"]') || e.target.closest('.overflow-x-auto') || e.target.closest('.overflow-y-auto')
    if (scrollable) {
  ...
  /* Fast-poll permissions every 15 s */
  ...
  /* Merge live store permissions with cached JWT permissions */
  ...
  const filteredNavItems = useMemo(
    () => getFilteredNavItems(employeeRole, mergedPermissions, hasAssignedCourses),
    [employeeRole, mergedPermissions, hasAssignedCourses]
  )

  /* RBAC path protection */
  useEffect(() => {
    const allowed = filteredNavItems.some(item =>
      item.path === '/employee' ? location.pathname === '/employee' : location.pathname.startsWith(item.path)
    )
    if (!allowed && location.pathname.startsWith('/employee')) navigate('/employee', { replace: true })
  }, [location.pathname, filteredNavItems, navigate])

  const currentNavItem = useMemo(() => {
    const byPath  = navItems.find(item => item.path === '/employee' ? location.pathname === '/employee' : location.pathname.startsWith(item.path))
  ...
  return (
    <div
      className="relative h-screen overflow-hidden flex text-slate-900"
      style={{ background: '#f8fafc', fontFamily: "-apple-system, BlinkMacSystemFont, 'Inter', sans-serif" }}
    >
      {/* Mobile backdrop */}
      {mobileMenuOpen && (
        <div
      ...
      {/* ── SIDEBAR ── */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col transition-all duration-300 ease-[cubic-bezier(0.4,0,0.2,1)] lg:static lg:translate-x-0
          ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}
          ${sidebarOpen   ? 'w-64'          : 'w-[72px]'}`}
        style={{ background: '#ffffff', borderRight: '1px solid #e2e8f0', boxShadow: '2px 0 16px rgba(15,23,42,0.05)' }}
      >
        {/* Brand */}
        <div
          className="flex items-center gap-3 px-4 overflow-hidden flex-shrink-0"
          style={{ borderBottom: '1px solid #f1f5f9', minHeight: 64 }}
        >
          <div
            className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl"
            style={{ background: 'linear-gradient(135deg,#3b82f6,#6366f1)', boxShadow: '0 4px 12px rgba(99,102,241,0.25)' }}
          >
          ...
          {sidebarOpen && (
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-[0.28em] text-blue-500">SolutionHub</p>
        ...
        {sidebarOpen && (
          <div
            className="mx-3 mt-3 rounded-2xl p-3 flex-shrink-0"
            style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}
          >
            <div className="flex items-center gap-2.5">
              <div
                className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl text-[13px] font-black text-white overflow-hidden"
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-bold text-slate-800">{employeeName}</p>
                <p className="truncate text-[11px] text-slate-400">{employeeEmail}</p>
              </div>
            </div>
            <div className="mt-2.5 flex flex-wrap gap-1.5">
              <span className="rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.2em] text-blue-600" style={{ background: '#eff6ff', border: '1px solid #bfdbfe' }}>
                {isTeamMember ? 'Team' : 'Employee'}
              </span>
              <span className="max-w-full truncate rounded-full px-2 py-0.5 text-[9px] font-semibold text-slate-500" style={{ background: '#f1f5f9', border: '1px solid #e2e8f0' }}>
                {employeeRole}
              </span>
            </div>
          </div>
          ...
          {filteredNavItems.map((item) => {
            ...
            return (
              <Link
                ...
                style={isActive ? { background: '#eff6ff', border: '1px solid #bfdbfe' } : { border: '1px solid transparent' }}
              >
                {isActive && <span className="absolute left-0 top-1/2 -translate-y-1/2 h-5 w-[3px] rounded-r-full bg-blue-500" />}

                <span
                  className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg transition-all"
                  style={isActive
                    ? { background: GRADIENTS[item.color] || 'linear-gradient(135deg,#3b82f6,#6366f1)', boxShadow: '0 2px 8px rgba(99,102,241,0.3)' }
                ...
                {isActive && sidebarOpen && <span className="ml-auto h-1.5 w-1.5 rounded-full bg-blue-500 flex-shrink-0" />}
              </Link>
            )
          })}
        </nav>
        ...
        {/* Logout */}
        <div className="p-2.5 flex-shrink-0" style={{ borderTop: '1px solid #f1f5f9' }}>
          <button
            onClick={handleLogout}
            className={`group flex w-full items-center gap-3 rounded-xl px-2.5 py-2.5 text-sm font-semibold text-slate-500 transition-all hover:bg-red-50 hover:text-red-600 ${sidebarOpen ? '' : 'justify-center'}`}
            style={{ border: '1px solid transparent' }}
          >
            <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg" style={{ background: '#fff1f2', border: '1px solid #fecdd3' }}>
        ...
        <button
          ...
          className="absolute -right-3.5 top-20 hidden h-7 w-7 items-center justify-center rounded-full text-slate-500 transition-all hover:text-slate-900 lg:flex"
          style={{ background: '#ffffff', border: '1px solid #e2e8f0', boxShadow: '0 2px 8px rgba(15,23,42,0.1)' }}
        >
      ...
      <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
        ...
        {/* Header */}
        <header
          className="flex-shrink-0 flex items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"
          style={{ background: '#ffffff', borderBottom: '1px solid #f1f5f9', boxShadow: '0 1px 6px rgba(15,23,42,0.05)', minHeight: 64 }}
        >
          {/* Left */}
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[9px] font-black uppercase tracking-[0.25em] text-blue-500 mb-0.5">{currentNavItem.label}</p>
              <h2 className="text-[16px] font-black text-slate-800 tracking-tight truncate">
                Welcome back, <span className="text-blue-600">{employeeName.split(' ')[0]}</span>
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            >
              ...
              <span className="text-[11px] font-semibold text-slate-600">{currentNavItem.label}</span>
            </div>
            ...
            <Link
              to="/employee/profile"
              className="flex items-center gap-2 rounded-xl px-2.5 py-1.5 transition-all hover:bg-slate-50"
              style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}
            >
              <div
                className="w-6 h-6 rounded-lg overflow-hidden flex items-center justify-center text-[10px] font-black text-white flex-shrink-0"
                style={{ background: 'linear-gradient(135deg,#3b82f6,#6366f1)' }}
              >
                {employeeAvatar ? <img src={employeeAvatar} alt={employeeName} className="w-full h-full object-cover" /> : employeeInitial}
              </div>
        ...
        {/* Page content */}
        <main
          className="flex-1 overflow-y-auto overscroll-contain px-4 py-5 sm:px-6 lg:px-8"
          onWheel={handleWheel}
