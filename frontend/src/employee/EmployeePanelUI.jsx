export function EmployeePageHeader({ eyebrow = 'Employee Panel', title, description, actions = null, stats = [] }) {
  return (
    <section className="relative overflow-hidden rounded-[30px] border border-slate-300 bg-[radial-gradient(circle_at_top_left,_rgba(56,189,248,0.18),_transparent_28%),radial-gradient(circle_at_bottom_right,_rgba(16,185,129,0.16),_transparent_24%),linear-gradient(135deg,_rgba(3,7,18,0.96),_rgba(15,23,42,0.94))] p-6 shadow-2xl shadow-black/20 lg:p-8">
      ...
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <h1 className="mt-3 text-3xl font-black tracking-tight text-slate-900 lg:text-4xl">{title}</h1>
          {description && <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">{description}</p>}
          {stats.length > 0 && (
            <div className="mt-5 flex flex-wrap gap-2">
              {stats.map(stat => (
                <div key={`${stat.label}-${stat.value}`} className="rounded-full border border-slate-300 bg-slate-100 px-3 py-1.5 text-xs text-slate-200">
...
export function EmployeeSurface({ title, description, action = null, children, className = '' }) {
  return (
    <section className={`rounded-[28px] border border-slate-300 bg-slate-950/70 p-5 shadow-2xl shadow-black/15 backdrop-blur-xl lg:p-6 ${className}`}>
...
export function EmployeeEmptyState({ 
  ...
  return (
    <div className={`rounded-[26px] border border-dashed border-slate-300 bg-white/[0.03] px-6 py-12 text-center ${className}`}>
...
export function EmployeeBadge({ children, tone = 'neutral' }) {
  const toneMap = {
    neutral: 'border-slate-300 bg-slate-100 text-slate-200',
