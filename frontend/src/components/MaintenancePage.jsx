...
export default function MaintenancePage({ message }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#030712] text-slate-100 flex flex-col justify-between items-center overflow-hidden font-['Outfit',sans-serif] selection:bg-indigo-500/30 selection:text-indigo-200">
      ...
      {/* Header Bar */}
      <header className="relative z-10 w-full max-w-7xl px-6 py-6 flex items-center justify-between">
        ...
        {/* Admin Portal Shortcut Link with SVG Lock Icon */}
        <Link
          to="/admin"
          className="flex items-center gap-2 px-4 py-2 text-xs font-semibold text-slate-300 bg-slate-900/80 hover:bg-slate-800/80 hover:text-white border border-slate-800 hover:border-slate-700 rounded-full transition-all duration-300 shadow-sm backdrop-blur-md"
        >
      ...
      <footer className="relative z-10 w-full max-w-7xl px-6 py-6 text-center text-xs text-slate-500 border-t border-slate-900/60">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <p>© {new Date().getFullYear()} Amit Solution Technology Pvt Ltd. All rights reserved.</p>
