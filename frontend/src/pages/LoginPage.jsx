...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (raw.includes('no user record') || raw.includes('provided identifier')) msg = 'No account found with this email.'
      setError(msg)
    } finally {
      setLoading(false)
    }
      ...
      <div className="relative z-10 w-full max-w-[440px]">
        ...
        <div className="relative rounded-[28px] border border-white/10 bg-white/[0.04] backdrop-blur-2xl p-8 sm:p-9 shadow-[0_24px_60px_-12px_rgba(0,0,0,0.8)]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Link to="/forgot-password?from=customer" className="text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors">
                  Forgot password?
                </Link>
              </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-3.5 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 rounded-2xl text-sm font-bold text-slate-300 hover:text-white transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {googleLoading ? (
