    ...
    <>
      ...
      >
        <PublicSection className="space-y-8">
          ...
          <div>
            <div className="text-center mb-10">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-300 text-xs font-bold uppercase tracking-wider mb-4">
                <span>🎓</span> Internship Programs
              </div>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white mb-3">
                Kickstart Your Career with Our Internships
              </h2>
              <p className="text-sm text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
                Industry-focused internship programs designed to give you real-world experience, professional certification, and career support.
              </p>
            </div>
            ...
            {loading ? (
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div key={i} className="h-64 animate-pulse rounded-[28px] border border-white/60 bg-white/50 dark:border-white/5 dark:bg-white/3" />
                ))}
              </div>
            ) : (() => {
              const internshipCourses = courses
                .filter(c => c.published !== false)
                .slice(0, 6)
              
              if (internshipCourses.length === 0) return null
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                  {internshipCourses.map((course) => {
                    ...
                    return (
                      <PublicGlassCard
                        key={course.id}
                        className="group flex flex-col p-6 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_28px_60px_-20px_rgba(99,102,241,0.25)]"
                      >
                        <div className="relative mb-4">
                          <div className="aspect-video rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800">
                            ...
                            <div className="w-full h-full flex items-center justify-center text-4xl" style={{ display: course.thumbnail || course.image ? 'none' : 'flex' }}>
                              🎓
                            </div>
                          </div>
                          {course.badge && (
                            <span 
                              className="absolute top-3 right-3 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider text-white shadow-lg"
                          ...
                          {course.highlighted && (
                            <span className="absolute top-3 left-3 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-amber-500 text-white shadow-lg animate-pulse">
                              🔥 Popular
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex-1">
                          {course.category && (
                            <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200/70 bg-slate-50/80 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-600 dark:border-white/8 dark:bg-white/4 dark:text-slate-400 mb-2">
                              <span className="h-1 w-1 rounded-full bg-blue-500" />
                              {course.category}
                            </span>
                          )}
                          <h3 className="text-base font-black text-slate-900 dark:text-white leading-tight mb-2 group-hover:text-blue-600 dark:group-hover:text-cyan-300 transition-colors">
                            {course.title}
                          </h3>
                        ...
                        <div className="mt-4 flex items-center justify-between pt-4 border-t border-slate-100 dark:border-white/5">
                          <div>
                            {course.price > 0 ? (
                              <span className="text-lg font-black text-slate-900 dark:text-white">₹{Number(course.price).toLocaleString('en-IN')}</span>
                            ) : (
                          ...
                          <Link
                            ...
                            className="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300"
                          >
            ...
            {courses.filter(c => c.published !== false).length > 6 && (
              <div className="text-center mt-8">
                <Link
                  to="/courses"
                  className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 text-white font-bold shadow-lg shadow-emerald-500/20 hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300"
                >
