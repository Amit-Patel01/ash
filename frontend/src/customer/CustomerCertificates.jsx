...
function CertificateCard({ certificate, template, currentUser, copiedId, onCopy }) {
  ...
  const handleDownload = async (format) => {
    try {
      ...
      console.error(`Failed to download certificate ${format}:`, error)
      window.alert(`Unable to download certificate ${format.toUpperCase()} right now.`)
    } finally {
      setDownloading('')
    }
  }
    ...
    >
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h2 className="mt-3 text-2xl font-black text-white">{certificate.courseName || template.title}</h2>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-slate-500">Certificate ID</p>
...
export default function CustomerCertificates() {
  ...
  const template = useMemo(() => mergeCertificateTemplate(certificateTemplate), [certificateTemplate])
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Your issued certificates will appear here.</p>
        </div>
        ...
        <div className="bg-gray-900/60 border border-white/5 rounded-3xl p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-2 max-w-md mx-auto">
            Complete your course milestones and your certificate will show up here with a verification link.
          </p>
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            {myCertificates.length} verified certificate{myCertificates.length !== 1 ? 's' : ''} ready to share, download, and verify
          </p>
        </div>
        <Link
          to="/customer/my-courses"
