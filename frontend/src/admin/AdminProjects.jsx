      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          {['all', 'active', 'hidden'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize whitespace-nowrap transition-all ${statusFilter === status ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100 border border-transparent'}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex bg-slate-100 border border-slate-300 rounded-xl overflow-hidden">
            <button onClick={() => setViewMode('grid')} className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-slate-200 text-slate-900' : 'text-slate-400 hover:text-slate-900'}`}>
            ...
            <button onClick={() => setViewMode('list')} className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-slate-200 text-slate-900' : 'text-slate-400 hover:text-slate-900'}`}>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredProjects.map(project => (
            <div key={project.id} className={`group bg-white backdrop-blur-sm border rounded-2xl p-5 hover:border-slate-300 transition-all duration-300 ${project.status === 'hidden' ? 'border-red-500/20 opacity-60' : 'border-slate-200'}`}>
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={() => openEdit(project)} className="p-1.5 rounded-lg text-slate-400 hover:text-blue-400 hover:bg-blue-500/10 transition-colors opacity-0 group-hover:opacity-100">
                  ...
                  <button 
                    ...
                    className="p-1.5 rounded-lg text-slate-400 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100 disabled:opacity-50"
                  >
              ...
              <h3 className="text-base font-semibold text-slate-900 mb-1">{project.title}</h3>
              <p className="text-sm text-slate-400 line-clamp-2 mb-3">{project.description}</p>

              <div className="flex items-center gap-3 mb-3 text-sm">
                <span className="text-slate-500">Project: <span className="text-slate-900 font-medium">₹{Number(project.price_project_only).toLocaleString('en-IN')}</span></span>
                <span className="text-slate-500">|</span>
                <span className="text-slate-500">Source: <span className="text-slate-900 font-medium">₹{Number(project.price_with_source).toLocaleString('en-IN')}</span></span>
              </div>

              <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-slate-500">Slug: <span className="text-slate-600 font-mono">{project.slug}</span></span>
                </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left border-b border-slate-200">
                <th className="px-6 py-3 text-xs font-medium text-slate-400 uppercase">Project</th>
                <th className="px-6 py-3 text-xs font-medium text-slate-400 uppercase">Category</th>
                <th className="px-6 py-3 text-xs font-medium text-slate-400 uppercase">Price (Project)</th>
                <th className="px-6 py-3 text-xs font-medium text-slate-400 uppercase">Price (Source)</th>
                <th className="px-6 py-3 text-xs font-medium text-slate-400 uppercase">Sales</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredProjects.map(project => (
                <tr key={project.id} className={`hover:bg-slate-50 transition-colors ${project.status === 'hidden' ? 'opacity-60' : ''}`}>
                  ...
                  <td className="px-6 py-4 text-sm text-slate-900 font-medium">₹{Number(project.price_project_only).toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4 text-sm text-slate-900 font-medium">₹{Number(project.price_with_source).toLocaleString('en-IN')}</td>
