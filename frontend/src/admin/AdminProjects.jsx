    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{projects.length} projects | {activeCount} active | {totalSales} total sales</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Projects', value: projects.length, color: 'from-blue-500 to-indigo-600' },
          { label: 'Active', value: activeCount, color: 'from-emerald-500 to-green-600' },
          { label: 'Total Sales', value: totalSales, color: 'from-amber-500 to-orange-600' },
          { label: 'Featured', value: projects.filter(p => p.is_featured).length, color: 'from-purple-500 to-violet-600' },
        ].map((stat, i) => (
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          {['all', 'active', 'hidden'].map(status => (
            <button key={status} onClick={() => setStatusFilter(status)} className={`px-3 py-1.5 rounded-lg text-xs font-medium capitalize whitespace-nowrap transition-all ${statusFilter === status ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}>
        ...
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Search..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 pr-3 py-2 w-48 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/50 transition-all" />
          </div>
          <div className="flex bg-white/5 border border-white/10 rounded-xl overflow-hidden">
            <button onClick={() => setViewMode('grid')} className={`p-2 transition-colors ${viewMode === 'grid' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}>
            ...
            <button onClick={() => setViewMode('list')} className={`p-2 transition-colors ${viewMode === 'list' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}>
      ...
      {viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {filteredProjects.map(project => (
            <div key={project.id} className={`group bg-gray-900/50 backdrop-blur-sm border rounded-2xl p-5 hover:border-white/10 transition-all duration-300 ${project.status === 'hidden' ? 'border-red-500/20 opacity-60' : 'border-white/5'}`}>
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${project.status === 'active' ? 'bg-emerald-400' : 'bg-red-400'}`} />
                  <span className={`px-2 py-0.5 rounded-md text-[10px] font-medium ${project.category_slug === 'basic' ? 'bg-green-500/10 text-green-400' : project.category_slug === 'medium' ? 'bg-amber-500/10 text-amber-400' : 'bg-purple-500/10 text-purple-400'}`}>
                    {project.category_name}
                  </span>
                  {project.is_featured && <span className="px-2 py-0.5 rounded-md text-[10px] font-medium bg-blue-500/10 text-blue-400">Featured</span>}
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => toggleStatus(project.id, project.status)} className={`p-1.5 rounded-lg transition-colors opacity-0 group-hover:opacity-100 ${project.status === 'active' ? 'text-gray-500 hover:text-amber-400 hover:bg-amber-500/10' : 'text-gray-500 hover:text-emerald-400 hover:bg-emerald-500/10'}`} title={project.status === 'active' ? 'Hide' : 'Show'}>
                  ...
                  <button onClick={() => openEdit(project)} className="p-1.5 rounded-lg text-gray-500 hover:text-blue-400 hover:bg-blue-500/10 transition-colors opacity-0 group-hover:opacity-100">
                  ...
                  <button 
                    ...
                    className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors opacity-0 group-hover:opacity-100 disabled:opacity-50"
                  >
              ...
              <div className="flex items-center gap-3 mb-3 text-sm">
                <span className="text-gray-400">Project: <span className="text-white font-medium">₹{Number(project.price_project_only).toLocaleString('en-IN')}</span></span>
                <span className="text-gray-400">|</span>
                <span className="text-gray-400">Source: <span className="text-white font-medium">₹{Number(project.price_with_source).toLocaleString('en-IN')}</span></span>
              </div>

              <div className="flex items-center justify-between pt-3 border-t border-white/5">
                <div className="flex items-center gap-3">
                  <span className="text-xs text-gray-400">Slug: <span className="text-gray-300 font-mono">{project.slug}</span></span>
                </div>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-sm font-medium text-emerald-400">{project.sales || 0} sales</span>
                </div>
              </div>
            </div>
          ))}
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left border-b border-white/5">
                ...
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Price (Project)</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Price (Source)</th>
                <th className="px-6 py-3 text-xs font-medium text-gray-500 uppercase">Sales</th>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredProjects.map(project => (
                <tr key={project.id} className={`hover:bg-white/[0.02] transition-colors ${project.status === 'hidden' ? 'opacity-60' : ''}`}>
                  <td className="px-6 py-4">
                    <p className="text-sm font-medium text-white">{project.title}</p>
                    <div className="flex items-center gap-2 mt-1">
                      {project.is_featured && <span className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-blue-500/10 text-blue-400">Featured</span>}
                    </div>
                  </td>
                  <td className="px-6 py-4"><span className={`px-2 py-0.5 rounded text-[10px] font-medium ${project.category_slug === 'basic' ? 'bg-green-500/10 text-green-400' : project.category_slug === 'medium' ? 'bg-amber-500/10 text-amber-400' : 'bg-purple-500/10 text-purple-400'}`}>{project.category_name}</span></td>
                  <td className="px-6 py-4 text-sm text-white font-medium">₹{Number(project.price_project_only).toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4 text-sm text-white font-medium">₹{Number(project.price_with_source).toLocaleString('en-IN')}</td>
                  <td className="px-6 py-4"><span className="text-sm text-emerald-400 font-medium">{project.sales || 0}</span></td>
                  <td className="px-6 py-4">
                    <button onClick={() => toggleStatus(project.id, project.status)} className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${project.status === 'active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                      {project.status}
                    </button>
                  </td>
                  <td className="px-6 py-4">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-gray-900 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <div className="sticky top-0 bg-gray-900 border-b border-white/5 px-6 py-4 flex items-center justify-between z-10">
              <h2 className="text-lg font-semibold text-white">{editingProject ? 'Edit Project' : 'Add New Project'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} required placeholder="e.g. Portfolio Website" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Short Description *</label>
                <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} required rows={2} placeholder="Brief description shown on project cards" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all resize-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Detailed Description</label>
                <textarea value={formData.long_description} onChange={e => setFormData({ ...formData, long_description: e.target.value })} rows={3} placeholder="Full description shown on project detail page" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all resize-none" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Price (Project Only) *</label>
                  <input type="number" value={formData.price_project_only} onChange={e => setFormData({ ...formData, price_project_only: e.target.value })} required placeholder="499" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-400 mb-1.5">Price (With Source) *</label>
                  <input type="number" value={formData.price_with_source} onChange={e => setFormData({ ...formData, price_with_source: e.target.value })} required placeholder="999" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Features (comma separated)</label>
                <input type="text" value={formData.features} onChange={e => setFormData({ ...formData, features: e.target.value })} placeholder="Responsive Design, Dark Mode, Contact Form" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5">Tech Stack (comma separated)</label>
                <input type="text" value={formData.tech_stack} onChange={e => setFormData({ ...formData, tech_stack: e.target.value })} placeholder="React, Tailwind CSS, Vite" className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
              </div>
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.is_featured} onChange={e => setFormData({ ...formData, is_featured: e.target.checked })} className="w-4 h-4 rounded border-white/20 bg-white/5 text-blue-500 focus:ring-blue-500/30" />
                  <span className="text-sm text-gray-300">Featured</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={formData.status === 'active'} onChange={e => setFormData({ ...formData, status: e.target.checked ? 'active' : 'hidden' })} className="w-4 h-4 rounded border-white/20 bg-white/5 text-emerald-500 focus:ring-emerald-500/30" />
                  <span className="text-sm text-gray-300">Active (visible on site)</span>
                </label>
              </div>
              <div className="flex items-center gap-3 pt-4">
                <button type="button" onClick={() => setShowModal(false)} disabled={uploading} className="flex-1 px-4 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-gray-300 hover:bg-white/10 transition-all">Cancel</button>
                <button type="submit" disabled={uploading} className="flex-1 px-4 py-2.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl text-sm font-medium text-white hover:shadow-lg hover:shadow-blue-500/25 transition-all flex items-center justify-center gap-2">
