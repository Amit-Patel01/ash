    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap justify-end">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl px-4 py-2 flex items-center gap-3">
            ...
            <span className="text-xs font-bold text-gray-300">{students.length} Total Students</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-black text-white">{students.filter(c => !isInactive(c)).length}</p>
          </div>
      ...
      <div className="bg-gray-900/50 backdrop-blur-xl border border-white/5 rounded-[2rem] overflow-hidden shadow-2xl">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                <th className="px-6 py-5 text-[10px] font-black uppercase tracking-widest text-gray-500">Student Info</th>
                ...
                <tr key={student.uid} className="hover:bg-white/[0.02] transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-sm font-bold text-white leading-tight">{student.displayName || 'Anonymous User'}</p>
                  ...
                  <td className="px-6 py-4">
                    <div className="space-y-1">
                      <p className="text-xs font-medium text-gray-400">ID: <span className="text-[10px] font-mono text-blue-400/80">{student.uid.slice(0, 8)}...</span></p>
                      <p className="text-[10px] text-gray-500 uppercase tracking-tighter">Registered: {student.createdAt ? new Date(student.createdAt).toLocaleDateString() : '—'}</p>
                    </div>
                  </td>
                  <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <h3 className="text-xl font-black text-white tracking-tight">Add student</h3>
            <p className="text-sm text-gray-400">Creates the account in the primary database and Firebase. The student receives an email to set a password.</p>
            <div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-hidden">
          ...
          <div className="relative bg-gray-900 border border-white/10 rounded-[2.5rem] w-full max-w-sm p-8 shadow-2xl animate-in zoom-in-95 duration-300 text-center">
            ...
            <h3 className="text-2xl font-black text-white tracking-tight">Delete Student?</h3>
            <p className="text-sm text-gray-400 mt-3 mb-8 leading-relaxed">
              Are you sure you want to permanently delete <span className="text-white font-bold text-base underline decoration-red-500/30">"{studentToDelete?.displayName || 'this account'}"</span>? This action cannot be reversed.
            </p>
