  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Student Testimonials</h1>
          <p className="text-sm text-gray-400 mt-1">{testimonials?.length || 0} reviews</p>
        </div>
      ...
      {(!testimonials || testimonials.length === 0) ? (
        <div className="bg-gray-900/50 border border-white/5 rounded-2xl p-14 text-center">
          ...
          <p className="text-gray-400 font-medium mb-3">No testimonials found</p>
          <p className="text-xs text-gray-500 max-w-sm mx-auto">Click "Add Testimonial" to create dynamic reviews that will display on the Home page.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {testimonials.map(t => (
            <div key={t.id} className="bg-gray-900/60 border border-white/5 rounded-2xl p-6 flex flex-col justify-between group hover:border-white/10 transition-all relative overflow-hidden">
              <div>
                {/* Header */}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-gray-900 border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
              <h2 className="text-lg font-bold text-white">{editing ? 'Edit Testimonial' : 'Add Testimonial'}</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-semibold text-gray-400 mb-1.5">Course / Role *</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">Avatar Emoji</label>
                <div className="flex flex-wrap gap-2">
                  {EMOJI_OPTIONS.map(emoji => (
                    <button type="button" key={emoji} onClick={() => setForm({...form, avatar: emoji})}
                      className={`w-10 h-10 flex items-center justify-center rounded-xl text-xl transition-all border ${form.avatar === emoji ? 'bg-blue-500/10 border-blue-500 text-white' : 'bg-white/5 border-white/5 text-gray-400 hover:bg-white/10 hover:text-white'}`}>
                      {emoji}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">Rating (1 to 5 Stars)</label>
                <select value={form.rating} onChange={e => setForm({...form, rating: Number(e.target.value)})}
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-blue-500/50">
                  <option value="5" className="bg-gray-950">5 Stars (⭐⭐⭐⭐⭐)</option>
                  <option value="4" className="bg-gray-950">4 Stars (⭐⭐⭐⭐)</option>
                  <option value="3" className="bg-gray-950">3 Stars (⭐⭐⭐)</option>
                  <option value="2" className="bg-gray-950">2 Stars (⭐⭐)</option>
                  <option value="1" className="bg-gray-950">1 Star (⭐)</option>
                </select>
              </div>
              ...
              <div>
                ...
                <textarea value={form.text} onChange={e => setForm({...form, text: e.target.value})} required rows={4} placeholder="What did this student say about their experience?"
                  className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-sm text-white focus:outline-none focus:border-blue-500/50 resize-none" />
              </div>
