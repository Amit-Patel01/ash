...
const ServiceForm = ({ formData, setFormData, onCancel, onSubmit, submitLabel }) => (
  <div className="space-y-4">
    <div>
      ...
      <input type="text" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Web Development" className="w-full px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
    </div>
    <div>
      <label className="block text-xs font-medium text-slate-500 mb-1.5">Description</label>
      <textarea value={formData.description} onChange={e => setFormData({ ...formData, description: e.target.value })} rows={2} placeholder="Brief description of the service" className="w-full px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all resize-none" />
    </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          <select 
            ...
            className="w-full px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-900 focus:outline-none focus:border-blue-500/50 transition-all"
          >
            <option value="code" className="bg-white">Code</option>
            <option value="tool" className="bg-white">Tool</option>
            <option value="video" className="bg-white">Video</option>
            <option value="support" className="bg-white">Support</option>
            <option value="other" className="bg-white">Other (Custom)</option>
          </select>
          {!['code', 'tool', 'video', 'support'].includes(formData.icon) && (
            <input 
              ...
              className="w-full px-4 py-2 bg-slate-100 border border-slate-300 rounded-xl text-xs text-slate-900 placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all animate-in fade-in slide-in-from-top-1" 
            />
          )}
        </div>
      </div>
      <div>
        ...
        <div className="space-y-2">
          ...
          >
            ...
            <option value="Support" className="bg-white">Support</option>
            <option value="Creative" className="bg-white">Creative</option>
            <option value="other" className="bg-white">Other (Custom)</option>
          </select>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1.5">Base Price (INR)</label>
        <input type="number" value={formData.basePrice} onChange={e => setFormData({ ...formData, basePrice: e.target.value })} placeholder="499" className="w-full px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
      </div>
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1.5">Redirect Path (e.g. /services/web)</label>
        <input type="text" value={formData.path} onChange={e => setFormData({ ...formData, path: e.target.value })} placeholder="/services/details" className="w-full px-4 py-2.5 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-gray-600 focus:outline-none focus:border-blue-500/50 transition-all" />
      </div>
    </div>
    <div className="flex justify-end gap-3 pt-2">
      <button onClick={onCancel} className="px-4 py-2 bg-slate-100 border border-slate-300 rounded-xl text-sm text-slate-600 hover:bg-slate-200 transition-colors">Cancel</button>
      <button onClick={onSubmit} className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl text-sm font-medium text-slate-900 hover:shadow-lg hover:shadow-blue-500/25 transition-all">{submitLabel}</button>
    </div>
  </div>
)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Services Platform</h1>
          <div className="flex items-center gap-3 mt-1.5">
            <p className="text-sm text-slate-400">{services.length} services offered</p>
        ...
        <button 
          ...
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-purple-600 rounded-xl text-sm font-bold text-slate-900 hover:shadow-lg hover:shadow-blue-500/30 hover:-translate-y-0.5 transition-all duration-200"
        >
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-blue-500/10 border border-blue-500/20 rounded-xl text-sm text-blue-300">
        ...
        These services are displayed globally across the main website. Students can request custom projects based on them.
      </div>
      ...
      {(showAddModal || editingService) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-50 border border-slate-300 rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="h-1 w-full bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-500" />
            
            <div className="px-6 py-5 border-b border-slate-200 flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-slate-900">{editingService ? 'Edit Service' : '✦ New Capability'}</h2>
                <p className="text-xs text-slate-400 mt-0.5">{editingService ? 'Update existing service catalog' : 'Add a new service offering to the platform'}</p>
              </div>
              <button onClick={() => { setShowAddModal(false); setEditingService(null) }} className="p-2 rounded-xl text-slate-400 hover:text-slate-900 hover:bg-slate-200 transition-all">
        ...
        {categories.map(cat => (
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${categoryFilter === cat
                ...
                : 'bg-slate-100 text-slate-500 hover:text-slate-900 hover:bg-slate-200 border border-transparent'
              }`}
          >
            {cat}
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          ...
          return (
            <div
              ...
              className={`relative group rounded-2xl overflow-hidden border transition-all duration-300 ${
                service.active 
                  ? 'border-slate-300 hover:border-white/20 shadow-lg hover:shadow-xl hover:-translate-y-1' 
              ...
              <div className="p-6">
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="px-2 py-0.5 bg-slate-100 border border-slate-300 rounded-lg text-[10px] font-medium text-slate-500">
                      {service.category}
                    </span>
                  </div>
                ...
                <div className="bg-black/30 rounded-xl p-3 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-400 font-medium text-xs">Starting Base Price</span>
                    <span className="text-slate-900 font-bold tracking-wide">₹{service.basePrice.toLocaleString('en-IN')}</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold mb-0.5">Orders</span>
                    ...
                    <div className="flex flex-col items-end">
                      <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold mb-0.5">Revenue</span>
                ...
                <div className="mt-4 flex items-center justify-between bg-slate-50 p-2.5 rounded-xl border border-slate-200 group-hover:border-slate-300 transition-colors">
                  <span className="text-xs font-semibold text-slate-500">{service.active ? 'Accepting Requests' : 'Currently Hidden'}</span>
        ...
        <button
          onClick={() => { setShowAddModal(true); setEditingService(null); setFormData({ name: '', description: '', icon: 'code', basePrice: '', category: 'Development', path: '', active: true }) }}
          className="group rounded-2xl border-2 border-dashed border-slate-300 hover:border-blue-500/40 bg-transparent py-14 flex flex-col items-center justify-center gap-4 text-slate-400 hover:text-blue-400 transition-all duration-300"
        >
          <div className="w-14 h-14 rounded-2xl bg-slate-100 group-hover:bg-blue-500/10 flex items-center justify-center transition-all">
