...
export function EmployeeDashboard({ employee = {}, onClose, createMode = false }) {
  const { updateUser, addUser, deleteUser } = useStore()
  ...
  return (
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        {/* ── Bottom Save & Account Action Bar ───────────────────────────────── */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pb-10 pt-4 border-t border-slate-200">
          {!createMode && (
            <div className="flex items-center gap-2 flex-wrap">
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    await updateUser(empId, { status: newStatus })
              ...
              <button
                type="button"
                onClick={async () => {
                  if (window.confirm(`Are you sure you want to permanently delete account for ${form.displayName || form.email}? This action cannot be undone.`)) {
                    try {
