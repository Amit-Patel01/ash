...
const Layout = ({ children }) => {
  ...
  try {
    ...
    const routerLocation = useLocation ? useLocation() : null
    pathname = nextPathname || routerLocation?.pathname || ''
  } catch (e) {
  ...
  const hideNavbar = hideShellRoutes.some(path => pathname.startsWith(path))
  const hideFooter = [...hideShellRoutes, '/admin', '/employee'].some(path => pathname.startsWith(path))
  const hideAnnouncement = pathname.startsWith('/verify')
  const hideGlowBackground = pathname.startsWith('/verify')

  // Hide WhatsApp & Back-to-top on dashboard routes
  const isPrivateRoute = ['/admin', '/employee', '/user'].some(p => pathname.startsWith(p))
