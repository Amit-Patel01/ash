 ...
 */
// ─── Primary & Fallback Backend URLs ──────────────────────────────────────────
export const RENDER_PRIMARY_URL = 'https://solutionhub-1.onrender.com'
export const DEVTUNNEL_FALLBACK_URL = (
  import.meta.env.VITE_DEVTUNNEL_URL ||
...
export const API_BASE = (() => {
  // 1. Explicit VITE_API_URL or VITE_BACKEND_URL from build/env
  ...
  if (isBrowser) {
    ...
    // 2. VS Code Devtunnel auto-detection (e.g. xxx-5173.inc1.devtunnels.ms -> xxx-5000.inc1.devtunnels.ms)
    if (host.includes('devtunnels.ms')) {
      const portPattern = /-[0-9]{4,5}\./
      if (portPattern.test(host)) {
        return `${protocol}//${host.replace(portPattern, '-5000.')}`
      }
      return `${protocol}//${host}`
    }

    // 3. Local Wi-Fi IP auto-detection (e.g. 192.168.x.x or 10.x.x.x)
  ...
  // 5. Production fallback to Render backend if no VITE_API_URL set
  if (import.meta.env.PROD) {
    return RENDER_PRIMARY_URL
  }
...
export async function fetchWithFailover(input, init = {}, nativeFetch = (typeof window !== 'undefined' ? window._originalFetch || window.fetch : fetch)) {
  ...
  // Only intercept requests directed to Render or Devtunnel or relative /api
  const isTargetingBackend = 
    rawUrl.includes('onrender.com') || 
    rawUrl.includes('devtunnels.ms') || 
    rawUrl.startsWith('/api') ||
    rawUrl === API_BASE ||
    rawUrl.startsWith(`${API_BASE}/`)

  if (!isTargetingBackend) {
    return nativeFetch(input, init)
  }

  // Build primary (Render) and fallback (Devtunnel) URLs
  ...
  if (rawUrl.includes('devtunnels.ms')) {
    ...
    primaryUrl = `${RENDER_PRIMARY_URL}${rawUrl}`
    fallbackUrl = `${DEVTUNNEL_FALLBACK_URL}${rawUrl}`
  }
